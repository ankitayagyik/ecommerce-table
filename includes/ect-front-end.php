...
if (!function_exists('ectp_generate_heading')) { 
    ...
    function ectp_generate_heading($ect_table_settings){
        ...
        if($ect_table_settings['ect_image'] == 'checked'){
            ...
            echo ectp_div_wrapper(esc_html('Image'), esc_attr('image_padding'));
        ...
        echo ectp_div_wrapper(
            ectp_div_wrapper(esc_html('Add To Cart')).
            ectp_div_wrapper(ectp_get_cart()), esc_attr('ectp_cart_align')
        );
...
if (!function_exists('ectp_get_cart')) { 

    function ectp_get_cart(){

        global $woocommerce;

        $ectp_cart = $woocommerce->cart->get_cart();

        $ectp_cart_count = 0;

        foreach($ectp_cart as $key => $values){

            $ectp_cart_count += $values['quantity'];

        }

        $cartimg = '<img class="'.esc_attr("ectp_ajax_cart_image").'" src="'.plugins_url( "../assets/cart.webp", __FILE__ ).'">';
        
        return $cartimg.'<span class="'.esc_attr("ectp_cart_contents_count ectp_ajax_cart").'" > '.esc_html($ectp_cart_count).'</span>';
    }
